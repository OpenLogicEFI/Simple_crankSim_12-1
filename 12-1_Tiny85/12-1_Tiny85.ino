...
int sensorPin = A1;              // potentiometer
int sensorValue = 0;            // store value from sensor

unsigned long val = 2;                    //value used with map
const int crankpin =  0;      // Crank sensor to ECU

int ledState = LOW;            // trigger
int ledState2 = LOW;           // dead area

unsigned long previousMicros = 0;      // will store last variable of ledstate
unsigned long interval = 50;          // interval at which to trigger (milliseconds)
...
void loop()
{
  ...
  interval = map(sensorValue, 0, 1023, 48, 2046);        // interval 0-1024 real/ x - x out
